...
class IndexFirstAxis(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input.scatter_(
            0,
            indices.unsqueeze(-1).expand(indices.size(0), grad_output.size(1)),
            grad_output,
        )
...
def pad_input(
    inputs: torch.Tensor, indices: torch.Tensor, batch: int, seqlen: int
) -> torch.Tensor:
        ...
        if seq_len > self.max_position_embeddings:
            ...
            if self.mixed_b is None:
                ...
                a = (
                    ...
                    / (self.dim / 2) ** self.mixed_b
                )  # (13)
                lambda_1_m = (
                    a
                    * torch.arange(1, self.dim // 2 + 1).float().to(device)
                    ** self.mixed_b
                ).exp()  # (12)
...
LAYER_NORM = {"layer_norm": nn.LayerNorm, "rms_norm": RMSNorm}
        ...
        if position_ids is None:
            ...
            if unpad_inputs:
                ...
                position_ids = torch.cat(
                    [self.position_ids[:l] for l in length]
                ).unsqueeze(0)
            else:
                # [bs, seq_len]
                position_ids = self.position_ids[:seq_length].expand(batch_size, -1)
        elif unpad_inputs:
            position_ids = position_ids[attention_mask_bool].unsqueeze(
                0
            )  # [1, cumsum_seq_len]
...
class NewAttention(nn.Module):
    def __init__(
        self, config: NewConfig, pack_qkv=None, use_memory_efficient_attention=None
    ):
        super().__init__()
        self.config = config
        ...
        if self.pack_qkv:
            self.qkv_proj = nn.Linear(
                config.hidden_size, self.all_head_size * 3, bias=True
            )
        else:
        ...
        self.memory_efficient_attention = (
            None if xops is None else xops.memory_efficient_attention
        )
        if self.use_memory_efficient_attention:
            assert (
                self.memory_efficient_attention is not None
            ), "please install xformers"
        if self.config.unpad_inputs:
            assert (
                self.config.use_memory_efficient_attention
            ), "unpad only with xformers"
    ...
    def forward(
        ...
        if self.config.position_embedding_type == "rope":
            query_states, key_states = apply_rotary_pos_emb(
                query_states, key_states, *rope_embeds
            )
        ...
        if self.use_memory_efficient_attention:
            ...
            ), "memory_efficient_attention do not output attentions"
    ...
    def _attention(
        self, query_states, key_states, value_states, attention_bias, head_mask
    ):
        """
        Args:
...
class NewGatedMLP(nn.Module):
    ...
    def __init__(self, config: NewConfig):
        ...
        self.up_gate_proj = nn.Linear(
            config.hidden_size, self.intermediate_size * 2, bias=False
        )
        self.down_proj = nn.Linear(
            self.intermediate_size, config.hidden_size, bias=True
        )
...
class NewEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.layer = nn.ModuleList(
            [NewLayer(config) for _ in range(config.num_hidden_layers)]
        )
...
class NewModel(NewPreTrainedModel):
    ...
    def forward(
        ...
        unpad_inputs = (
            unpad_inputs if unpad_inputs is not None else self.config.unpad_inputs
        )
        ...
        if unpad_inputs:
            ...
            attention_bias = xops.fmha.attn_bias.BlockDiagonalMask.from_seqlens(
                length, device=self.device
            )
        else:
            # We can provide a self-attention mask of dimensions [batch_size, from_seq_length, to_seq_length]
            # ourselves in which case we just need to make it broadcastable to all heads.
            attention_bias = self.get_extended_attention_mask(
            ...
            if self.config.use_memory_efficient_attention:
                ...
                attention_bias = attention_bias.expand(
                    -1, self.config.num_attention_heads, seq_length, -1
                )

        if self.config.logn_attention_scale:
            # attention scale log_512(input_len)
            attention_scale = (
                attention_mask.sum(1).log()
                / torch.tensor(self.config.max_position_embeddings).log()
            )
